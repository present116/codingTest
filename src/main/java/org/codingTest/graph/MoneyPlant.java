...
public class MoneyPlant {
    ...
    public int solution(int n, int[][] flights, int s, int e, int k){
        ...
        for(int[] x : flights) {
            ...
            // graph.get() => ArrayList<int[]>
            // ArrayList<int[]> .add() -> int~~
        }

        Queue<int[]> Q = new LinkedList<>();
        Q.offer(new int[]{s, 0}); // s => 현수가 사는 도시 - cost
        costs[s] = 0;
        int L = 0;
        ...
        if(costs[e] == 1000000000) return -1; // e 목적지
        else return costs[e];

    }

    public static void main(String[] args){
        MoneyPlant T = new MoneyPlant();
